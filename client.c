...
void Recv_Message() 
{
  ...
  { 
    ...
    if (receive > 0) 
    {
      ...
      //distinguish them one by one 
      int rv=sscanf(message,"%s %s %s",get_Protocol,getUserName,getMessage);

      char dest[LENGTH]={0};
      memcpy(dest, message+3,sizeof(message));
      //check the MSG 4 Characters and compare with info in get_protocol
      ...
      //error handling
      else if (strcmp(get_Protocol,"ERROR")==0)
      {
        if(strcmp(getUserName,username)==0)
        {
          printf("%s ERROR 255 characters allowed  \n",username);
          fflush(stdout);
        }
...
//check commandline argurmnets 
	if(argc<3)
	{
		printf("Please enter IP:Port and a nick name in that order \n");
    fflush(stdout);
		exit(0);
	}
...
// checking the asda nicknames
  ...
  for(int i=2;i<argc;i++)
  {
    if(strlen(argv[i])<12)
    {
      ...
      if(!reti)
      {
        //c,ear username, check for validity using regular expressions
      ...
      else 
      {
        //error if username is not valid
	      printf("%s is NOT valid User Name.\n",argv[i]);
        fflush(stdout);
        exit(0);
      }
    } 
    ...
    {  
      //the nick name should be less than 12 characters    
      printf("%s is too long (%zu vs 12 chars).\n", argv[i], strlen(argv[i]));
      fflush(stdout);
      exit(0);
    }
  }
